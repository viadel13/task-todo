...
export const TaskContainer = () => {
  ...
  const addTask = (title) => {
    const newTask = {
      id: tasksList.length ? tasksList[tasksList.length - 1].id + 1 : 1, // permet de donner le nombre d'élément ajouté
      title: title,
      completed: false,
    };
    const updatedTasks = [...tasksList, newTask];
    localStorage.setItem("task-todo", JSON.stringify(updatedTasks));
  ...
  const editTask = (id, completedValue) => {
    setTasksList(
      tasksList.map(
        (
          task, // .map permet d'afficher les éléments d'un tableau dynamiquement
  ...
  return (
    <main>
      ...
      {/*permet d'envoyer la methode à utiliser à ce composant enfant*/}
      <TaskList
        tasksList={tasksList}
        editTasK={editTask}
